...
メンバ関数は、書かれた仮引数のほかに、`.` の左のオブジェクトを **オブジェクト引数** として受け取る。オブジェクト引数を書き換えないメンバ関数には `const` を付けられ、これを **const メンバ関数** と呼ぶ。const なオブジェクトや const 参照の先では、const メンバ関数だけが呼べる。読み取るだけの操作を const にしておくと、そうした場面でも使え、書き換える操作と区別もつく。
