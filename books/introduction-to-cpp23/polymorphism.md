...
継承の章では、`Counter` をもとに派生クラスを作った。本章では、基底クラスの参照やポインタを通して、派生クラスごとに異なる振る舞いを呼び分ける **多態**(polymorphism)を扱う。
...
多態には、`count_up` のように基底クラスの参照(またはポインタ)で扱うことが要る。基底クラスの値にコピーすると、派生クラスの部分が落ちてしまう。
...
`Counter c = b;` で `b` を `Counter` にコピーすると、`Counter` の部分だけが移り、`StepCounter` の部分は落ちる。`c` は `Counter` なので、`c.increment()` は `Counter` の `increment` になり、`count` は `1` になる。`StepCounter` の数え方は失われている。派生クラスの部分が落ちるこの現象を **スライシング** と呼ぶ。多態では、値ではなく参照やポインタで扱う。

## 仮想デストラクタ

多態のために継承の基底とするクラスには、デストラクタを `virtual` にしておくとよい。基底クラスのポインタを通してオブジェクトを破棄する場面で関わる。その破棄の場面は、`new` と `delete` を扱う後の章で見る。

## まとめ

基底クラスの参照やポインタを通すと、指している先の **動的な型** に応じて、呼ばれる関数が選ばれる。これを **多態** と呼ぶ。基底クラスで関数を `virtual`(仮想関数)にし、派生クラスで `override` を付けて **オーバーライド** する。`override` は、上書きの取り違えをビルドの段階で捕まえる。値で受け取ると派生クラスの部分が落ちる(スライシング)ため、多態には参照かポインタを使う。多態のための基底クラスには、デストラクタを `virtual` にしておくとよい(詳しくは後の章)。
