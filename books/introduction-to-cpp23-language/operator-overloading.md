...
`+` や `==` は、メンバ関数のほか、二つの引数を取る非メンバの関数としても定義できる。左右の二つの被演算子が、二つの引数になる。
...
:::details メンバ関数としてしか定義できない演算子
メンバでも非メンバでも定義できるかは、演算子によって決まる。`+` や `==` はどちらでも定義できるが、なかには非メンバにはできず、メンバ関数としてしか定義できない演算子もある。代入の `=`、添字の `[]`、関数呼び出しの `()`、メンバアクセスの `->`、そして変換演算子がそれにあたる。本章で扱う変換演算子も、メンバ関数として定義する。
:::
...
変換コンストラクタに `explicit` を付けて暗黙の変換を防げたのと同じく、変換演算子にも `explicit` を付けられる。`explicit` を付けると、この変換は明示的に書いたときだけ行われる。明示的な変換の書き方は、後の他のキャストの章で扱う。
