...
:::details プライベートモジュールフラグメント
グローバルモジュールフラグメントがモジュールの宣言の前に置かれるのに対し、`module :private;` の行から後ろは、モジュールインターフェース単位の末尾に置く **プライベートモジュールフラグメント** になる。ここに置いた宣言は、このモジュールを `import` した側からは見えない。`export` した名前の定義だけをここに移すと、宣言だけを前に残し、定義を取り込む側から隠せる。

```cpp:greeter.cppm
export module greeter;

export int answer();          // 宣言だけを前に置く

module :private;
int answer() { return 42; }   // 定義はここに隠す
```
...
```
`answer` の宣言は `import` した側から見えるが、その定義はプライベートモジュールフラグメントにあり、取り込む側からは見えない。

プライベートモジュールフラグメントは、本章で扱った他の機能と違い、執筆時点の g++ ではまだ使えない(`sorry, unimplemented: private module fragment` となる)。上のコードは clang で確かめた。
:::
