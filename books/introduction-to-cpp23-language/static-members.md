...
クラスのメンバには、オブジェクトごとに持つものと、クラスに1つだけ持つものがある。後者には `static` を付ける。本章では、クラスに1つの値を持つ `static` データメンバと、特定のオブジェクトに属さない `static` メンバ関数を扱う。
...
`static` メンバ関数はオブジェクト引数を取らないので、特定のオブジェクトに属する非 `static` のデータメンバは使えない。クラスに属する `count` のような `static` データメンバは使える。
