...
仮想関数の章で、仮想関数を持つ基底クラスのデストラクタを `virtual` にすると述べた。その理由が、この `delete` にある。
...
仮想関数を持つ基底クラスのデストラクタを `virtual` にするのは、この破棄を正しく行うためである。

## 確保と破棄を引き受ける道具へ

`new` と `delete` を正しく対にするのは、書く側の責任になる。`delete` を忘れればリークし、すでに `delete` したオブジェクトをもう一度 `delete` すれば、やはり結果は定まらない。
...
:::details 動的なオブジェクトを誰が破棄するか
動的に作ったオブジェクトをいつ破棄するかを、誰が決めるかは言語によって異なる。C++ では、`delete` を書く人が決めるか、寿命の尽きるときに自動で `delete` する道具(スマートポインタ)に任せる。Java や Python、Go では、使われなくなったオブジェクトを処理系が見つけて自動で回収する仕組み(ガベージコレクション)があり、破棄を自分では書かない。Rust では、各オブジェクトに持ち主が決まっていて、その持ち主の寿命が尽きるときに破棄される。C++ は破棄の時点を自分で決められる点が、これらと異なる。
:::
