...
前章の `std::ranges` のアルゴリズムは、範囲を巡って、並べ替えたり探したりした。範囲から別の範囲を導きたいこともある。偶数だけを残す、各要素を二倍にする、といった具合である。それを担うのが **ビュー** と、ビューを作る **レンジアダプタ** である。使うには `<ranges>` を取り込む。
...
ビューは、`std::span` と同じく、元の範囲を所有せず参照する。そのうえ、作った時点では計算しない。範囲for で巡るときに、要素ごとに計算する。途中の結果をためる別のコンテナは作らず、ビューは計算の手順を持つだけである。
...
`std::views::iota(1, 4)` は `1` から `4` の手前までの範囲、`v | std::views::take(3)` は `v` の先頭3つのビューである。

## コンテナに戻す

ビューは所有せず、遅延する。結果を持つコンテナがほしいときは、`std::ranges::to`(C++23)でビューをコンテナにする。
...
ビューは、`std::span` や `std::string_view` と同じく、元の範囲を所有しない。元の範囲が破棄されれば、ビューはもう存在しない範囲を見る。
...
ビューは、範囲から範囲を、遅延して導いた。次章では、こうした範囲やビューを支えるイテレータが、C++20 でどう捉え直されたかを見る。
