...
ここまで、値を持ち、参照し、所有するための型を見てきた。ここからは、一つの値を、それが持ちうる形とともに表す型を見る。値があるかないか(`std::optional`)、値かエラーか(`std::expected`)、いくつかの型のうちどれか(`std::variant`)、任意の型か(`std::any`)である。最初は、値があるかないかを表す `std::optional` を扱う。
