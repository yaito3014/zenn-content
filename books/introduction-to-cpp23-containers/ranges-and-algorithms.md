...
:::details 右辺値の範囲を渡すと、イテレータの代わりに dangling が返る
`std::ranges::find` のように範囲へのイテレータを返すアルゴリズムに、その場限りの範囲(関数が値で返したコンテナなど)を渡すと、戻ったイテレータは、すぐ破棄される範囲を指すことになる。`std::ranges` は、これをビルドの時点で防ぐ。

```cpp
std::vector<int> make() { return {1, 2, 3}; }

auto it = std::ranges::find(make(), 2);   // it の型は std::ranges::dangling
```

`make()` が返した `std::vector` は、この行の後で破棄される。`std::ranges::find` は、こうした範囲には本物のイテレータを返さず、`std::ranges::dangling` という型の値を返す。`dangling` はイテレータとして使えないので、`*it` と書くとビルドに失敗する。実行時に破棄された範囲を指すかわりに、ビルドの時点で弾かれる。

`std::span` や `std::string_view`、ビュー、あるいは名前のついたコンテナのように、指す先が別に保たれる範囲では、`find` はこれまでどおりイテレータを返す。
:::
...
`std::ranges` のアルゴリズムは、コンテナを巡って並べ替えたり探したりする操作だった。こうした操作は、コンテナのイテレータが何をできるかに支えられている。
