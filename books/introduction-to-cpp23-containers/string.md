...
個々の文字は、添字で取り出せる。`message[0]` は先頭の文字 `H` である。添字は `0` から始まり、`size()` 未満までが範囲の内にある。

`[]` は、添字が範囲の内にあるかを確かめない。範囲の外を渡すと、文字列の外を読むことになる。
...
`std::string` は、文字の並びを持ち、その格納を引き受ける型だった。次章からは、文字に限らず、同じ型の値の並びを持つコンテナ(`std::vector` と `std::array`)を見ていく。
