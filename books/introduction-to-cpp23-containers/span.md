...
本章では、要素を所有せずに範囲を参照する `std::span` を扱う。`std::span` は、文字列の章で見た `std::string_view` の考え方を、文字に限らない要素へ広げたものである。
...
`std::span` は、一次元の範囲を所有せずに参照した。行と列のような多次元の範囲を、同じく所有せずに参照する `std::mdspan`(C++23)もあるが、この編では扱わない。
