...
`std::variant` が挙げた型のうち一つを持つのに対し、型を挙げずにどんな型でも持てるのが `std::any` である。
...
どんな型の値でも、その型を表に出さずに一様に持つしくみを **型消去** と呼ぶ。`std::any` は、持っている型を表に出さないが、その型は内部に記録されており、`std::any_cast` が取り出しのたびに、要求した型がそれと合うかを実行時に確かめる。
